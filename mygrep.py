...
def setup_parser():
    ...
    parser.add_argument('files', metavar='PATTERN', nargs='*', default=['-'],
                        help='the files(s) to search')
    parser.add_argument('--color', '--colour', action='store_true',
                        help='highlight matches')
    parser.add_argument('-d', '--no-filename', action='store_true',
                        help='print without filename headers')
    parser.add_argument('-i', '--ignore-case', action='store_true',
                        help='case-insensitive search')
    parser.add_argument('-c', '--files-with-matches', action='store_true',
                        help='print only filenames with matches')
    parser.add_argument('-x', '--line-number', action='store_true',
                        help='print line numbers, indexed from 1')
    parser.add_argument('-R', '-r', '--recursive', action='store_true',
                        help='recursively search directories')
    parser.add_argument('-k','--search-key',action="store_true",
                        help='search on key of JSON')
    parser.add_argument('-v','--search-value',action="store_true",
                        help='search on value of JSON')
    return parser
